...
## Merchant Sheet:
Creating a merchant allows you to make stores that the players can visit or containers with items for the players to find on the map.  Create a Merchant actor and set their sheet to the Merchant Sheet.  This Merchant will be your master template, you need to make all players an owner of this actor so that they can interact with the sheets.  Do not place any items on the template Merchant, instead make sure that the token setting link actor data is un-checked.  This will mean when you place a merchant on a map that token will become the merchant, place items onto the tokens character sheet and set buy and sell prices per token.  Doing it this way makes it so players can only interact with stores on the maps that you place them on.
